...
The admin Pricing tab (`app/routes/app.pricing.$productId.tsx`) already lets a merchant configure a base price, per-answer "extra prices," and equations — but none of this data is connected to anything customer-facing. The customer-facing configurator (`app/routes/configurator.$productId.tsx`) loads the same `productConfig.options.pricing` object today but never reads it. The storefront's "Add to Cart" flow (`extensions/product-configurator/blocks/configurator_block.liquid`) always adds the product's plain Shopify variant at its listed price, regardless of what the customer picked. Confirmed live: a product with a $78 base price and a $30 "red" extra price checks out at exactly $78 no matter what the customer selects.
...
This function is the single source of truth: it's used both to render the live on-screen total and, later, to compute the authoritative charge server-side. No price value is ever trusted from the client for the actual charge.
...
- Currency: this route currently makes no Shopify Admin API calls (it's public/unauthenticated), so currency is passed in via a new `currency` query param on `configurator_url`, sourced from Liquid's `shop.currency` in `configurator_block.liquid` — the same pattern already used for the existing `shop` param.
- **Add to Cart button behavior branches on `pricingActive`:**
  ...
  - `true` (new): the iframe calls its own backend directly — `POST /configurator/$productId?shop=...` with `{ selectedAnswers, textValues, ... }` — then, on success, sets `window.top.location.href = invoiceUrl` to navigate the whole page out of the iframe to Shopify's hosted checkout for the resulting draft order.

### Server action (added to `configurator.$productId.tsx`)

1. Parse the POST body for `selectedAnswers` (and any other inputs that affect price/customization).
...
4. Get an admin client for the shop via `unauthenticated.admin(shop)` (exported from `shopify.server.ts`, using the shop's stored offline access token — same mechanism validated earlier today for the metafield write).
5. Call `draftOrderCreate` with one custom line item: `title` = product name, `originalUnitPrice` = computed total, `quantity` = 1, `customAttributes` = the same property list used today, `taxable` = `pricing.displayTaxes`.
...
- No rate-limiting beyond confirming the shop/product are real and pricing is configured for that product; repeated abuse of the endpoint could create empty draft orders in the merchant's admin. Flagged as a follow-up, not solved here.
